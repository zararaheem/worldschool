...
import { bulkSyncApplicationsToSheet } from '@/lib/google'
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await bulkSyncApplicationsToSheet(apps || [])
    return NextResponse.json({ written: result.written })
  } catch (err) {
    console.error('Sheet sync failed:', err)
    return NextResponse.json({ error: err instanceof Error ? err.message : 'Sheet sync failed.' }, { status: 500 })
  }
}
